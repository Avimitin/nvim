...
[auto-pairs](https://github.com/jiangmiao/auto-pairs) 是一个自动帮你补全括号和跳转括号的插件，默认不需要设置，不过偶尔会遇到不想要的跳转情况：
...
在上述情况，你可能想输入一个 `}` 来把括号补上，但是 auto-pairs 的跳转功能会直接帮你跳转到下一行的大括号而不是插入新的括号，在这种情况下，输入 `<Ctrl>+b` 就会跳回去并帮你把这个大括号补好：
